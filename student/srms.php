...
class srms
{
	public $base_url = 'http://localhost/sbadmin_astronomyapp/';
	public $connect;
	public $query;
	public $statement;
	public $now;
	...
	function Get_total_subject()
	{
		...
		FROM subject_srms 
		WHERE subject_status = 'Habilitado'
		";
		$result = $this->get_result();
		foreach($result as $row)
		{
	...
	function Get_total_student()
	{
		...
		FROM student_srms 
		WHERE student_status = 'Habilitado'
		";
		$result = $this->get_result();
		foreach($result as $row)
		{
